...
var generateListObject = exports.generateListObject = function generateListObject(_ref2) {
  var list = _ref2.list,
      offset = _ref2.offset,
      gap = _ref2.gap,
      minPercent = _ref2.minPercent;

  var copyList = list.map(function (item, index) {
    return Object.assign({}, item, { value: parseFloat(item.value) || 0, index: index });
  });
  var w = offset.w,
      h = offset.h,
      x = offset.x,
  ...
  var minValue = maxValue * minPercent;
  ...
  var filterMinList = copyList.filter(function (item) {
    return item.value <= minValue;
  });
  var maxLimitValue = _.max(_.map(filterMinList, 'value'));

  filterMinList.forEach(function (item) {
    var index = item.index,
        value = item.value;

    var filterPercent = value / maxLimitValue * 0.1 + 0.1;
    copyList[index].createValue = maxValue * filterPercent;
  });

  copyList.forEach(function (item, index, array) {
    var currentValue = item.createValue || item.value || 0;
    var after = array[index + 1] || {};
    var afeterValue = after.createValue || after.value || 0;
    ...
    if (len === 1 || index < len - 1) {
      ...
      if (len > 1) {
        ...
        var rate = (parseFloat(after.value) / parseFloat(item.value) * 100).toFixed(2);
        // ((bottomPercent / topPercent) * 100).toFixed(2)
        var tateText = '\u8F6C\u5316\u7387 ' + rate + '%';
...
var getSliceIndex = function getSliceIndex() {
  var text = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
  var vaildateFunc = arguments[1];

  var isExceed = vaildateFunc(text);
  if (isExceed) {
    var nextText = text.slice(0, -1);
    return getSliceIndex(nextText, vaildateFunc);
  }
  return _.size(text);
};

var getTextList = exports.getTextList = function getTextList(text, vaildateFunc) {
  var textList = [];
  var isExceed = vaildateFunc(text);
  var len = _.size(text);
  if (isExceed) {
    ...
    var start = 0;
    while (size < len) {
      var end = getSliceIndex(text.slice(start, len), vaildateFunc) + start;
      textList.push(text.slice(start, end));
      start = end;
      size = _.size(textList.reduce(function (p, c) {
        return p + c;
      }));
    }
  } else {
